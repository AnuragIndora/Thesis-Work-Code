...
# Function to construct adjacency matrix from landmarks
def construct_adjacency_matrix(landmarks):
    """Create an adjacency matrix based on Delaunay triangulation."""
    ...
    if not edges:
        ...
        if num_landmarks > 4:
            for i in range(0, num_landmarks, num_landmarks // 4):
                edges.append((i, (i + num_landmarks // 2) % num_landmarks))
...
# Save best model
def save_checkpoint(state, filename="best_simple_gcn.pth"):
...
plt.tight_layout()
plt.savefig('simple_gcn_training_curves.png', dpi=300, bbox_inches='tight')
plt.show()

# Load the best model for final evaluation
checkpoint = torch.load("best_simple_gcn.pth")
...
plt.tight_layout()
plt.savefig('simple_gcn_confusion_matrix.png', dpi=300, bbox_inches='tight')
plt.show()
...
def visualize_landmark_predictions(dataset, model, device, num_samples=6):
    ...
    plt.tight_layout()
    plt.savefig('simple_gcn_landmark_predictions.png', dpi=300, bbox_inches='tight')
    plt.show()
...
}, 'final_simple_gcn_model.pth')

print("Final model saved as 'final_simple_gcn_model.pth'")

test_dataset = pd.read_csv("tif_df_test.csv")
